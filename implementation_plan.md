...
12.5. ✅ **completed in v2 step 22** — **Tiered conversational memory.** See `v2_implementation_plan.md` for the build notes. `message_embeddings` writes on append (fire-and-forget `asyncio.create_task` until arq lands in v2 step 23); `thread_summaries` rows produced by `workers/jobs/compact_thread.py` with L1 windows + L2 summary-of-summaries (L1 stamped with `folded_into_summary_id` on fold). `conv.fetch_summaries` / `conv.search_relevant` wired into Triage + Quick + Planner. Seed skills landed alongside step 12.
...
17. ✅ **completed** — **Soul + User File integration.** `soul.md` and per-user `user_profiles` row into every system prompt; `soul_version` and `user_profile_version` on threads. Web UI for the user to view/edit their profile; Markdown body + structured preferences form. Triage Agent uses it for tone and routing. _New `persona/` subpackage: `soul.py` loads + hashes `soul.md` (12-char SHA-256 prefix is the version; `WOLFPAW_SOUL_PATH` config overrides the default repo-root lookup); `user_profile.py` DAO with partial-write `update(...)` that bumps `version` on every change; `builder.py` ships `build_system_prompt(...)` (pure) + `build_for_agent(...)` (runtime helper, defensive — falls back to bare agent_role on missing Soul or DB failure). All 5 agents renamed inline `_SYSTEM_PROMPT` → `_AGENT_ROLE` and now build the full system prompt at call time via `build_for_agent`; `prompt_versions` hashes only the role (the dynamic Soul + Profile blocks vary per-user). Triage's role prompt now references "the User File above" for tone + routing decisions. `memory.conversational.get_or_create_thread` stamps `soul_version` + `user_profile_version` on new threads (both nullable for degraded paths). REST endpoints `GET /me/profile` + `PATCH /me/profile` ship; the React UI for editing lands in step 19._
18. ✅ **completed** — **Telegram channel.** `@WolfpawBot`, webhook, `channel_links`, deep-link onboarding. All slash commands work here. `002_channels.sql`. _Migration `006_telegram.sql` (the original `002_channels.sql` slot was claimed by auth; same tables, later number) adds `channel_links` + `channel_link_tokens`. New `memory/channel_links.py` DAO (unique on `(channel, external_id)` so one Telegram account links to at most one Wolfpaw user). New `channels/telegram_client.py` with `HttpTelegramClient` (httpx wrapper over `sendMessage`) + `FakeTelegramClient` for tests + `set_telegram_client(...)` injection hook. `channels/telegram_tokens.py` mirrors `magic_link_tokens` — single-use, TTL'd, plaintext in the URL / SHA-256 hash on disk. `channels/telegram.py` ships `TelegramChannel` + two routes: `POST /channels/telegram/link-token` (authenticated Wolfpaw user → deep-link URL) and `POST /channels/telegram/webhook` (validates `X-Telegram-Bot-Api-Secret-Token`, parses Telegram updates, handles `/start link_<token>` onboarding, dispatches slash commands inline, runs the Router in a fire-and-forget `asyncio.create_task` for free-form messages — webhook returns 200 fast). Per-user thread continuity: each Telegram inbound extends the user's most recent telegram thread. **Deferred**: arq async processing (same fire-and-forget trade as web), Markdown formatting (plain text only), file uploads / voice / inline keyboards / group chats, proactive task-completion push (substrate is here via `TelegramChannel.send` but no caller yet)._
19. ✅ **completed** — **Web app.** React + Vite. Onboard, chat, task list, artifact browser, usage dashboard, channel settings. _New `web/` directory with Vite + React + TypeScript + React Router scaffold. AuthContext via session cookie + `GET /auth/me`; `RequireAuth` route guard; SignInPage (magic-link request) + VerifyPage (consumes `?token=...`). ChatPage with a custom POST-based SSE parser (native EventSource is GET-only) that handles every event type the backend emits, including an inline `ask_user` reply form that POSTs to `/channels/web/answer`. TasksPage + TaskDetailPage with cancel button. FilesPage (workspace list + per-row download via signed URL). ProfilePage (User File editor — persona_md / preferences / timezone — bumps version on save) + Telegram link-minting widget. UsagePage with scope tabs (default / today / month / all) rendering tables (charts deferred). Backend additions: `src/wolfpaw/tasks/routes.py` (`GET /tasks`, `GET /tasks/{id}`, `POST /tasks/{id}/cancel`) and `src/wolfpaw/metering/routes.py` (`GET /usage?scope=...`) — JSON companions to the slash commands, used by the React app. **Deferred**: file upload UI, charts on usage, visual design / theming (minimal CSS only), frontend tests (no vitest setup), PWA / offline. I haven't run `npm install` to verify the build; the user runs the dev server. The backend tests (247 pass) cover the new API contract end-to-end._
19.5. ✅ **completed** — **Tier 1 deferred-work cleanup** (after step 19). Six small, no-new-infra polish items spread across the agent + channel stack:
    - **`/reset` slash command.** Built-in alongside `/help`. `CommandResult` gained a `clear_thread: bool = False` flag; the web channel emits a new `reset` SSE event (before the usual `command` event) so the React client drops its `thread_id` and the next message creates a fresh thread server-side. Telegram (no client-side thread state) creates a new thread synchronously via `conv.get_or_create_thread(thread_id=None)` so the next message picks it up via the most-recent-thread resolver.
    - **TTL'd profile cache.** `persona/builder.py` now caches `UserProfile` per-user with a 30s in-process TTL (`_load_profile_cached`); `invalidate_profile(user_id)` drops the entry synchronously and is called from the PATCH endpoint. TTL is short enough that cross-process staleness from a PATCH on another worker self-heals within seconds. Cuts the DAO calls from N-per-agent-call down to ~1-per-burst.
    - **Per-root subagent concurrency cap.** New `MAX_CONCURRENT_SUBAGENTS_PER_ROOT = 5` semaphore in `agents/executor.py`, keyed on the root task id (resolved via new `memory.tasks.get_root`, mirrors `get_depth`). All descendants of one root share the cap, so a wide fanout plan can't saturate the model provider or sandbox pool; unrelated user tasks are independent. The semaphore registry is refcounted — entries drop when the last in-flight subagent releases.
...
20. ✅ **completed** — **OSS packaging (v1.5 entry).** Docker Compose (incl. Docker-based sandbox runtime for self-host), install script, README, env-template. _New top-level `Dockerfile` (two-stage Python 3.12-slim build; pip-installs the package; optional `[pdf]` / `[s3]` / `[e2b]` extras via `--build-arg EXTRAS=...`; runs as non-root uid 1000 with workspace + soul mounted into known paths). New `web/Dockerfile` (Vite build → nginx with SPA fallback) + `web/nginx.conf` (proxies `/auth /channels /workspace /tasks /usage /me /health` to the `app` service; `proxy_buffering off` for the SSE chat endpoint). New top-level `docker-compose.yml` with four services — `db` (`pgvector/pgvector:pg16` with healthcheck), `migrate` (one-shot, depends on `db` being healthy, runs `python -m scripts.migrate`), `app` (depends on `migrate` having completed successfully), `web` (publishes nginx on host `:3000` via `WOLFPAW_WEB_PORT`). Two named volumes: `wolfpaw_db` (Postgres data) and `wolfpaw_workspace` (LocalStorage files). New `scripts/migrate.py` is an idempotent runner — tracks applied files in a `schema_migrations` table, applies the diff in lexical order, leaves a failing migration unmarked so a fix + retry runs only it. New `WOLFPAW_MIGRATIONS_DIR` env override in `memory.db.migrations_dir()` so the resolver works both in the source tree and in the container. New `.env.example` covers every Wolfpaw env var with sane defaults + REQUIRED / OPTIONAL annotations. New `install.sh` is idempotent — copies `.env` from the template on first run, generates a real `WOLFPAW_SECRET_KEY` (via `openssl` or `python3 -c secrets.token_hex(32)`), warns about missing required keys, then runs `docker compose up -d --build` (skip with `NO_UP=1`). New `docs/self-host.md` is the full reference: stack diagram, env-var matrix, "behind a real domain" notes (Caddy snippet, Telegram webhook HTTPS), operations (logs / migrations / backup / update / reset), explicit "what's NOT in OSS" list, troubleshooting. New top-level `.dockerignore` + `web/.dockerignore` keep build context small. README's "Trying it locally today" section split into a Docker quickstart (recommended) + the direct dev workflow (for contributing). **Not validated**: I wrote the files but didn't run `docker compose up` — the user runs the end-to-end validation. The Python tests still pass (268 unit + 103 DB-gated skipped) confirming the code-side changes (`migrations_dir` env override) don't regress the dev path._
21. ✅ **completed** — **Slack (v2).** OAuth workspace install, app manifest, slash command + DMs. _New migration `007_slack.sql` adds `slack_workspaces` (per-workspace bot token + installer + soft-delete `revoked_at`); the existing `channel_links` + `channel_link_tokens` tables (from 006) cover the per-user mapping + the OAuth state token. New `memory/slack_workspaces.py` DAO with idempotent `upsert` (re-install rotates the token + clears revoked), `get` (filters revoked), `revoke`. New `channels/slack_signing.py` does HMAC-SHA256 verification against the raw request body with a ±5min replay window. New `channels/slack_client.py` ships `HttpSlackClient` (httpx → `oauth.v2.access` + `chat.postMessage`) + `FakeSlackClient` for tests + `set_slack_client(...)` injection hook; `SlackApiError` wraps non-ok responses. New `channels/slack.py` mounts four routes: `GET /channels/slack/install-url` (authed → mints state token via `channel_link_tokens` + returns Slack OAuth URL), `GET /channels/slack/oauth/callback` (consumes state → exchanges code → persists workspace + channel_link, renders a small HTML page with "Return to Wolfpaw"), `POST /channels/slack/events` (URL-verification handshake + DM dispatch — filters bot-authored messages + non-IM channels, fires Router in background), `POST /channels/slack/commands` (slash `/wolfpaw` — dispatches `/help`-style sub-commands inline; for free-form text returns "Working on it…" ephemeral ack and pushes the real reply via chat.postMessage when the Router finishes). Channel identity is composite `<team_id>:<slack_user_id>` so the same person in two workspaces is two distinct identities. Form-encoded slash command bodies parsed manually via `urllib.parse.parse_qs` to avoid pulling in `python-multipart` as a dep. New `docs/slack-app-manifest.yaml` is the operator-facing app manifest (paste into api.slack.com/apps "Create from manifest", replace `<YOUR-DOMAIN>`). New Slack settings in `config.py` (`slack_client_id`, `slack_client_secret`, `slack_signing_secret`, `slack_install_token_ttl_minutes`). `.env.example` documents the three required Slack env vars. **Deferred**: `app_mention` events in channels (substrate is here — scope claimed at install, handler is a 10-line addition), Slack thread_ts ↔ Wolfpaw thread_id mapping for in-channel continuity, mrkdwn formatting helpers (Slack's mrkdwn is permissive enough that plain agent output renders reasonably). 26 new tests added (8 signing + 13 channel HTTP + 5 DAO DB-gated). Suite total: 294 passing + 108 DB-gated skipped._
