...
- **`executor.py`** — `ExecutorAgent`: runs a `Plan`. Walks steps in execution order, batches contiguous parallel-group steps via `asyncio.gather`. Five step kinds: functional dispatches through `_resolve_tool` (builtin registry first, then user-tool fallthrough via `memory.tools.find_active_by_name` — v2 step 28); reasoning + evaluation make Sonnet calls with the plan + prior step results in context; **subagent** spawns a child Task via `TaskService.create_and_run` (step 16 — depth capped at 3, parent_task_id propagated, budget passed through); **tool_creator** invokes the Tool Creator agent on a Planner-spotted gap (v2 step 28). Subagent steps honour `inputs.on_failure ∈ {"fail" (default), "drop", "retry"}` (v2 step 27): `drop` returns a stub output marked `dropped=True` so synthesis can carry on with the surviving branches; `retry` re-spawns once with the failure diagnosis appended to the query. On step failure (other than `drop`-policy subagents): marks remaining steps `SKIPPED`, returns an error summary as the final answer. Tears down the task's sandbox in `finally`. Persists `final_answer` + `success` + `error` to procedural memory (Post-Evaluator follows up with `score`). Synthesis is skipped when the last completed step is reasoning (the planner already produced the final text). Singleton accessor `get_executor_agent()`.
...
- **`tool_creator.py`** — `ToolCreatorAgent` (v2 step 28): Sonnet with a *forced* `propose_tool` tool_use producing `{name, description, input_schema, implementation}`. The `create_tool(...)` method is the public entry point — invoked by the Executor when the Planner emitted a `tool_creator` step. Pipeline: propose → embed → cosine-dedup against existing approved user-tools (threshold `WOLFPAW_TOOL_DEDUP_SIMILARITY_THRESHOLD`, default 0.85) → persist as `proposed` → ask_user → mark `approved`/`rejected`. Returns `ToolCreationOutcome(status, tool_id, name, message)`. Gated by `WOLFPAW_TOOL_CREATOR_ENABLED` (default `True`). Singleton accessor `get_tool_creator_agent()`.
