...
- **Vector recall scope** — `search_relevant` is per-thread (`WHERE thread_id = $1`), but since threads are now channel-agnostic that already means "the user's whole continuous conversation." Touch `message_embeddings` lookups carefully — the IVFFlat index needs a `VACUUM ANALYZE` after large backfills to stay useful (memory-improvement step 4 swaps this for HNSW, which doesn't).
