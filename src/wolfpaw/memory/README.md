...
- **`task_events.py`** — append-only DAO over `task_events`. `append_event(conn, *, task_id, event_type, content)` writes one row. Step 14 ships the writer; step 15 will add the reader (`fetch_for_task`) and the task-status transition emitters. Migration `005_post_evaluator.sql` made `task_id` nullable so the Post-Evaluator can emit `plan_scored` events for plans that aren't (yet) wrapped in a Task; the originating plan id is carried in `content`.
