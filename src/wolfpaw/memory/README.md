...
flowchart TD
    ...
    subgraph Trio
        ...
        T3["conv.search_relevant (recency-weighted recall + windows)"]
...
- **Vector recall scope** — `search_relevant` is per-thread (`WHERE thread_id = $1`), but since threads are now channel-agnostic that already means "the user's whole continuous conversation." `message_embeddings` uses an **HNSW** index (`018_message_embeddings_hnsw.sql`) — high recall-at-latency into the millions of vectors, no clustering assumption, and no post-backfill `VACUUM ANALYZE` step (unlike the IVFFlat indexes still used by `plans` / `skills` / `tools` / `workspace_files`). Query-time recall/speed is governed by the `hnsw.ef_search` GUC (default 40) if you ever need to tune it.
