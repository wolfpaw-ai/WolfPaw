...
- **`tasks.py`** — DAO for `tasks` with the full state machine: `create`, `get_by_id` (user-scoped), `list_for_user` (newest first, optional status filter), `mark_started` / `mark_awaiting_user` / `mark_blocked` / `mark_completed` / `mark_failed` / `cancel`, `attach_plan`. Terminal status (completed/failed/cancelled) is sticky — subsequent transition attempts no-op rather than corrupting state. `cancel` and `get_by_id` are user-scoped so one user can't kill or inspect another's task.
